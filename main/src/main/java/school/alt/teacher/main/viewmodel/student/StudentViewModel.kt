...
//                }
    ...
    fun modifyStudent(userdata: EditUserRequest, onAction: () -> Unit) {
        viewModelScope.launch {
            kotlin.runCatching { teacherRepository.editStudent(userdata) }
                .onSuccess { result ->
                    Log.d("studentViewModel", "resutl : $result")
                    withContext(Dispatchers.Main) {
                        onAction()
                    }
                }
    ...
    //  과연 안드에서 하는게 비 효율 적인가?
    //  아무리 생각해 봐도 클라 단에서 웹, 앱 둘다 하는데
    // 클라에서 2번 처리하는거
                ...
                "이름만 입력 했을 때 ex) 엄성민 or 엄 or 성 or 엄성" -> {
                    when (_userListUiState.value) {
                        is UserListUiState.Success -> {
                            val list =
                                (_userListUiState.value as UserListUiState.Success).studentList
                                        ...
                                        if (it.user.name?.contains(keyword) == true) {
                                            if (it.user.className != 0) {
                                                newSearchMap[it.user.grade]?.get(it.user.className - 1)
                                                    ?.add(it)
                                                newSearchMap[0]?.get(it.user.className - 1)?.add(it)
                                            } else {
                                                newSearchMap[0]?.get(it.user.className)?.add(it)
                                                newSearchMap[it.user.grade]?.get(it.user.className - 1)
                                                    ?.add(it)
                                            }
                                        }
                        ...
                        is UserListUiState.Success -> {
                            ...
                            list.forEach { key, value ->
                                value.forEach {
                                    it.forEach {
                                        val data = it.user
                                        val studentNumber = String.format(
                                            "%02d%02d%02d",
                                            data.grade,
                                            data.className,
                                            data.classNo
